...
from evaluation.utils.plotting import (
    ...
    plot_static_reconstruction_accuracy
)
...
class CVAEEvaluator:
    ...
    def _create_output_directories(self) -> str:
        ...
        # Handle both 'output' and 'outputs' keys for backward compatibility
        output_config = self.config.get('output', self.config.get('outputs', {}))
    ...
    def load_model(self, checkpoint_path: Optional[str] = None) -> None:
        ...
        data_path = self.config['data'].get('data_path', self.config['data'].get('processed_data_path'))
        ...
        # Create model - detect if hierarchical by checking checkpoint keys
        checkpoint_keys = set(checkpoint['model_state_dict'].keys())
        is_hierarchical = any('global_encoder' in key or 'local_encoder' in key for key in checkpoint_keys)
        
        if is_hierarchical:
            # Create HierarchicalCVAE
            self.model = HierarchicalCVAE(
    ...
    def load_data(self) -> None:
        ...
        # Create dataloader
        dataloader_config = self.config['data'].get('dataloader', self.config['data'])
        ...
        with torch.no_grad():
            for batch_idx, batch in enumerate(tqdm(self.dataloader, desc="Evaluating")):
                ...
                try:
                    ...
                    if isinstance(self.model, HierarchicalCVAE):
                        # For hierarchical models, use the full forward pass
                        model_outputs = self.model(signals, static_params)
                        reconstructed = model_outputs['recon_signal']
                        predicted_peaks = model_outputs.get('predicted_peaks', None)
                        # Use combined latent for visualization
                        mu_global = model_outputs['mu_global']
                        mu_local = model_outputs['mu_local']
                        mu = torch.cat([mu_global, mu_local], dim=1)
                    else:
                        # Standard CVAE approach
                        # Encode
    ...
    def generate_samples(self) -> torch.Tensor:
        ...
        generation_config = self.config.get('visualization', {}).get('generation', {})
        num_conditions = generation_config.get('num_conditions', 5)
        samples_per_condition = generation_config.get('samples_per_condition', 3)
        ...
        generated_static_from_model = []  # NEW: For static params generated by the model
        ...
        with torch.no_grad():
            for i in range(num_conditions):
                ...
                for _ in range(samples_per_condition):
                    # Handle different model architectures
                    if isinstance(self.model, HierarchicalCVAE):
                        # Sample from hierarchical latent spaces
                        sample_outputs = self.model.sample(
                            static_params=condition_static,
                            n_samples=1
                        )
                        generated = sample_outputs['generated_signals']
                        
                        # FIXED: Extract generated static parameters from model
                        if 'generated_static_params' in sample_outputs:
                            generated_static_from_model.append(sample_outputs['generated_static_params'].cpu())
                        
                    else:
                        # Sample from prior
                        z = torch.randn(1, self.model.latent_dim).to(self.device)
                        
                        # Check if model supports joint generation
                        if hasattr(self.model, 'joint_generation') and self.model.joint_generation:
                            decoder_output = self.model.sample(static_params=condition_static, n_samples=1)
                            if isinstance(decoder_output, tuple) and len(decoder_output) >= 2:
                                generated, generated_static = decoder_output[:2]
                                generated_static_from_model.append(generated_static.cpu())
                            else:
                                generated = decoder_output
                        else:
                            # Legacy mode
        ...
        # NEW: Save generated static parameters from model if available
        if generated_static_from_model:
            generated_static_from_model = torch.cat(generated_static_from_model, dim=0)
            self.results['data']['generated_static_params_from_model'] = generated_static_from_model
            self.logger.info(f"Generated {len(generated_samples)} samples with {len(generated_static_from_model)} static parameter sets")
        else:
            self.logger.info(f"Generated {len(generated_samples)} samples (no static parameters generated by model)")
    ...
    def create_visualizations(self) -> None:
        ...
        reconstruction_config = viz_config.get('reconstruction', {})
        if reconstruction_config.get('enabled', True):
            ...
            num_samples = min(reconstruction_config.get('num_samples', 10), len(original))
            ...
            try:
                ...
                self.logger.warning(f"Could not create reconstruction comparison plot: {e}")
                # Create empty plot file to continue
                import matplotlib.pyplot as plt
                plt.figure(figsize=(10, 6))
                plt.text(0.5, 0.5, f"Reconstruction plot failed: {e}", ha='center', va='center')
        ...
        latent_config = viz_config.get('latent_space', {})
        if latent_config.get('enabled', True):
            ...
            max_samples = latent_config.get('num_samples', 5000)
            
            # Handle color_by parameter (could be string or list)
            color_by = latent_config.get('color_by', 'intensity')
            ...
            try:
                plot_latent_space_2d(
                    ...
                    method=latent_config.get('method', 'pca'),
                    color_by=color_by,
                    max_samples=max_samples
                )
            except Exception as e:
                self.logger.warning(f"Could not create latent space plot: {e}")
                # Create empty plot file to continue
                import matplotlib.pyplot as plt
                plt.figure(figsize=(10, 6))
                plt.text(0.5, 0.5, f"Latent space plot failed: {e}", ha='center', va='center')
                plt.savefig(save_path)
                plt.close()
        ...
        generation_viz_config = viz_config.get('generation', {})
        if generation_viz_config.get('enabled', True):
            ...
            try:
                plot_generation_samples(
                    ...
                    generation_viz_config.get('samples_per_condition', 3)
                )
            except Exception as e:
                self.logger.warning(f"Could not create generation samples plot: {e}")
                # Create empty plot file to continue
                import matplotlib.pyplot as plt
                plt.figure(figsize=(10, 6))
                plt.text(0.5, 0.5, f"Generation samples plot failed: {e}", ha='center', va='center')
                plt.savefig(save_path)
                plt.close()
        ...
        # NEW: 6. Generated static parameters visualization
        if 'generated_static_params_from_model' in self.results['data']:
            self.logger.info("  Creating generated static parameters plot...")
            
            generated_static = self.results['data']['generated_static_params_from_model']
            original_static = self.results['data']['generation_static_params']
            
            save_path = os.path.join(self.output_dir, 'plots', 'generated_static_params.png')
            
            try:
                plot_static_reconstruction_accuracy(
                    generated_static.numpy(), 
                    original_static.numpy(), 
                    save_path, 
                    title="Generated vs Input Static Parameters"
                )
                
                self.results['visualizations']['generated_static_params'] = save_path
                
            except Exception as e:
                self.logger.warning(f"Could not create generated static parameters plot: {e}")
                # Create empty plot file to continue
                import matplotlib.pyplot as plt
                plt.figure(figsize=(10, 6))
                plt.text(0.5, 0.5, f"Generated static params plot failed: {e}", ha='center', va='center')
                plt.savefig(save_path)
                plt.close()
    ...
    def save_results(self) -> None:
        ...
        output_config = self.config.get('output', self.config.get('outputs', {}))
        if not output_config.get('save_summary', True):
            return
        ...
        with open(summary_path, 'w') as f:
            ...
            # NEW: Generated static parameters metrics
            if 'generated_static_params_from_model' in self.results['data']:
                f.write("Generated Static Parameters:\n")
                f.write("-" * 20 + "\n")
                generated_static = self.results['data']['generated_static_params_from_model']
                original_static = self.results['data']['generation_static_params']
                
                # Basic statistics
                f.write(f"  Number of generated static parameter sets: {generated_static.shape[0]}\n")
                f.write(f"  Static parameter dimensions: {generated_static.shape[1]}\n")
                f.write(f"  Generated static range: [{generated_static.min():.4f}, {generated_static.max():.4f}]\n")
                f.write(f"  Generated static mean: {generated_static.mean():.4f}\n")
                f.write(f"  Generated static std: {generated_static.std():.4f}\n")
                
                # Compare with input static parameters
                if original_static.shape == generated_static.shape:
                    mse_static = torch.nn.functional.mse_loss(generated_static, original_static).item()
                    mae_static = torch.nn.functional.l1_loss(generated_static, original_static).item()
                    f.write(f"  MSE vs input static params: {mse_static:.6f}\n")
                    f.write(f"  MAE vs input static params: {mae_static:.6f}\n")
                f.write("\n")
